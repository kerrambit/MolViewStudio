...
export class Ipc {
  static Electron = class {
    static handle<Key extends keyof EventPayloadMapping>(key: Key, handler: () => EventPayloadMapping[Key]) {
    ...
    static send<Key extends keyof EventPayloadMapping>(key: Key, payload: EventPayloadMapping[Key], window: BrowserWindow) {
